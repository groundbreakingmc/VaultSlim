...
public class Vault extends JavaPlugin {
    ...
    private <T> T getProvider(Class<T> providerClass) {
        ...
        final String name = System.getProperty("vault." + providerClass.getSimpleName(), null);
        final RegisteredServiceProvider<T> provider = name == null
                ? sm.getRegistration(providerClass)
                : sm.getRegistrations(providerClass).stream()
                .filter(r -> r.getPlugin().getName().equalsIgnoreCase(name))
                .findFirst()
                .orElse(null);

        if (provider != null) {
            super.getLogger().info(providerClass.getSimpleName() + " provider loaded by " + provider.getPlugin().getName() + " (" + provider.getProvider().getClass().getCanonicalName() + ")");
            return provider.getProvider();
        } else {
            super.getLogger().warning(providerClass.getSimpleName() + " provider not found!");
            return null;
        }
    }
}
